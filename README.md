...
- **Play into an app (v2.1.1):** a recorded macro can play straight into one app in the background — pick it under **Play into** in the Recorder. Clicks and keys reach that app while you use another; your cursor never moves.
...
|---|---|
| Normal Mac apps (AppKit) | ✅ Works — clicks, keys and typed text (macro **Play into** re-measured 2026-09-26) |
...
- **Follow the window:** every recorded click captures the app that owns the window it was clicked in, and that window's position. With **Follow the window** on (default), replay moves each click (press and release together) by how far that window has moved, so a macro recorded on a window in one place keeps working after you move it. Turn it off for exact recorded screen positions. If the app or its window is gone at replay, the run stops with a warning instead of clicking wherever the old coordinates now point. Recordings made before this feature (and clicks whose window couldn't be identified) simply replay at their recorded positions.
- **Play into (background macros):** in the Recorder, set **Play into** to an app and press Play — there's no countdown, and every click and key goes straight into that app's window while you keep working (or playing) in another. Scrolls and cursor moves are skipped, because the cursor stays yours. Before its first click into each of its windows, the app gets a focus nudge (browser-type apps need it to accept clicks), so the app you're using may lose focus for a moment — a game in front may pause once. Roblox is refused up front: it ignores input unless it's the front app (measured), so play Roblox macros normally, with Roblox in front. To run a macro on a **browser game while you use other tabs**, drag that game's tab out into its own window and play into the browser.
